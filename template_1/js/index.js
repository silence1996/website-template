$(document).ready(function() {
	...
	function viewNav(el, w){
		if(w <= 960){
			...
			if($('.h-container').hasClass('h-container-active')){
				$('.h-container').removeClass('h-container-active');
			}
			if($('.h-mask').hasClass('h-mask-active')){
				$('.h-mask').removeClass('h-mask-active');
			}
			if($('.index-nav-btn span').hasClass('inb-active')){
				$('.index-nav-btn span').removeClass('inb-active');
			}
		}else{
